    ...
    def do_POST(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix='partners-dropbox-sync-') as td:
                ...
                # Required live sources. Purchase Order Summary has been retired.
                for key in ('opportunities','orderbook','targets','podates'):
                    stage=f'download:{key}'
                    paths[key]=download_one(key,td)
                    progress.append({'stage':stage,'ok':True,'name':DROPBOX_SOURCES[key]['name']})

                # Strategic Partners is useful but intentionally non-blocking.
                key='strategic'
                stage=f'download:{key}'
                try:
                    paths[key]=download_one(key,td)
                    ...
                    progress.append({'stage':stage,'ok':False,'warning':str(e),'optional':True})
