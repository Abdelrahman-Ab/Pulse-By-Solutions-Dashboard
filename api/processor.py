...
def build_all(paths: dict, source: str = 'dropbox', previous_meta: dict | None = None):
    ...
    purchase_order_details = _run('process:po-details-business', data_engine.load_po_details_rows)
    strategic_partners = _run('process:strategic-partners', data_engine.load_strategic_partners)

    # Purchase Order Summary has been retired. Reconstruct the compact relationship
    # dataset directly from Purchase Order Details, which is now the single PO source.
    purchase_orders, po_matches = _run(
        'process:po-relationships-from-details',
        data_engine.load_po_from_details,
        purchase_order_details
    )
    if not purchase_orders:
        raise DatasetError(
            'process:po-relationships-from-details',
            'Purchase Order Details could not produce the purchase order relationship dataset.'
        )
    po_summary_mode = 'purchase-order-details-only'

    # Enrich Order Book rows with Vendor / Partner relationships derived from PO Details.
    ...
    meta = {
        ...
        'purchaseOrderDetails': len(purchase_order_details),
        'strategicPartners': len(strategic_partners),
        'purchaseOrderDateMatches': po_matches,
        'purchaseOrderSummaryMode': po_summary_mode,
        'purchaseOrderDateLookupSize': len(po_date_map),
        'roadmapOrder': data_engine.ROADMAP_ORDER,
        'source': source,
        'buildSeconds': round(time.time() - started, 2),
        'pipelineVersion': 'partners-performance-v25-no-po-summary',
    }
