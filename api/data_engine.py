...
import os, json, math, zipfile, xml.etree.ElementTree as ET, argparse, re
        ...
        with z.open(sheet) as fh:
            ...
            for ev,e in ET.iterparse(fh,events=('end',)):
                if not e.tag.endswith('}row'):
                    continue
                row_ordinal += 1
                # Some Power BI/Excel exports omit row r= references entirely.
                # Fall back to physical row order without changing behavior for normal XLSX files.
                r=int(e.attrib['r']) if e.attrib.get('r') else row_ordinal
                if r>scan_rows:
                    e.clear(); break
                ...
                if best is None or candidate[:4] > best[:4]:
                    best=candidate
                # Fast path: when the preferred header row is clearly the export header,
                # avoid scanning thousands of data rows just to rediscover the same table.
                if preferred_header_row and r==preferred_header_row and score>=8:
                    e.clear()
                    return sheet,r,raw,norm_to_canonical
                e.clear()
...
def load_po_details_rows():
    """Load the Business with Partners fact table directly from Purchase Order Details.

    The user-approved display fields are mapped from the exact export columns where possible,
    with tolerant aliases for normal Excel/Power BI header variations.  The Purchase Order
    Details workbook is the sole analytical source for the Business with Partners section.
    """
    aliases = {
        'Accounting Date': {'Accounting Date','AccountingDate','PO Date','Purchase Order Date','Date'},
        'Market Unit': {'Marketunit','Market Unit','MarketUnit','vue_PO[New Marketunit]','New Marketunit','New Market Unit'},
        'Country': {'Country','vue_PO[Country]'},
        'Company': {'Company','vue_PO[Company]','Legal Entity','LegalEntity'},
        'Vertical': {'Vertical','vue_PO[Vertical]','vue_PO[New Vertical Name]','New Vertical Name'},
        'Sector': {'Sector','vue_PO[Sector]'},
        'Subsector': {'Subsector','SubSector','vue_PO[Subsector]','vue_PO[SubSector]'},
        'Customer Name': {'Customer Name','vue_PO[Customer Name]','Customer'},
        'Contract Name': {'Contract Name','vue_PO[Contract Name]'},
        'Contract ID': {'Contract ID','vue_PO[Contract ID]'},
        'Project ID': {'Project ID','ProjectID','PROJECTID','vue_PO[Project ID]'},
        'Purchase Order': {'Purchase Order','PurchaseOrder','PO','PO Number','Purchase Order Number','vue_PO[Purchase Order]'},
        'OpportunityID': {'OpportunityID','Opportunity ID','Opp ID','OppID','vue_PO[OpportunityID]','vue_PO[Opportunity ID]'},
        'Submitting User': {'Submitting User','vue_PO[Submitting User]'},
        'Buyer': {'Buyer','buyer','Orderer','Purchaser','vue_PO[Orderer]'},
        'Supplier Name': {'Supplier Name','Supplier  Name','Supplier','vue_PO[Supplier Name]'},
        'Vendor Name': {'Vendor Name','Partner Name','vue_PO[Vendor Name]','vue_PO[Partner Name]'},
        'Vendor Type': {'Vendor Type','Partner Type UPDATED','Partner Type Updated','Partner Type','vue_PO[Partner Type UPDATED]'},
        'Vendor ID': {'Vendor ID','Partner_Key','Partner Key','PartnerKey','vue_PO[Partner_Key]'},
        'PO SAR': {'PO SAR','PO SAR Amount','Purchase Order SAR','Purchase Order SAR Amount','NET Amount SAR','NET Line Amount SAR','Net Amount SAR','Net SAR','[NET_Line_Amount_SAR_Measure]'},
        'Gross SAR': {'Gross SAR','Gross Amount SAR','Gross Line Amount SAR','[Gross_Line_Amount_SAR_Measure]'},
    }
    wanted=set().union(*aliases.values())
    canonical_by_norm={_header_norm(a):canon for canon,vals in aliases.items() for a in vals}
    ...
    for r in iter_records(FILES['podates'], 1, wanted):
        row={canonical_by_norm.get(_header_norm(k),k):v for k,v in r.items()}
        po=clean(row.get('Purchase Order'))
        d=excel_iso(row.get('Accounting Date'))
        # Keep useful detail rows even when PO is absent; filters/charts can still use them.
        if not po and not any(clean(row.get(k)) for k in ('Contract ID','OpportunityID','Partner Name','Supplier Name')):
            continue
        y=year_from_iso(d) if d else None
        q=q_from_iso(d) if d else 'Unassigned'
        m=m_from_iso(d) if d else 'Unassigned'
        po_sar=num(row.get('PO SAR'))
        gross_sar=num(row.get('Gross SAR'))
        # If the export only exposes the gross SAR measure, do not silently render a zero chart.
        if not po_sar and gross_sar:
            po_sar=gross_sar
        out.append({
            'accountingDate':d,
            'accountingYear':y,
            'accountingQuarter':q,
            'accountingMonth':m,
            'market':clean(row.get('Market Unit'),'Unassigned'),
            'country':clean(row.get('Country'),'Unassigned'),
            'company':clean(row.get('Company'),'Unassigned'),
            'vertical':clean(row.get('Vertical'),'Unassigned'),
            'sector':clean(row.get('Sector'),'Unassigned'),
            'subsector':clean(row.get('Subsector'),'Unassigned'),
            'customerName':clean(row.get('Customer Name'),'Unassigned'),
            'contractName':clean(row.get('Contract Name'),'Unassigned'),
            'contractId':clean(row.get('Contract ID'),'Unassigned'),
            'projectId':clean(row.get('Project ID'),'Unassigned'),
            'purchaseOrder':po or 'Unassigned',
            'opportunityId':clean(row.get('OpportunityID'),'Unassigned'),
            'submittingUser':clean(row.get('Submitting User'),'Unassigned'),
            'buyer':clean(row.get('Buyer'),'Unassigned'),
            'supplierName':clean(row.get('Supplier Name'),'Unassigned'),
            'vendorName':clean(row.get('Vendor Name'),'Unassigned'),
            'vendorType':clean(row.get('Vendor Type'),'Unassigned'),
            'vendorId':clean(row.get('Vendor ID'),'Unassigned'),
            'poSAR':po_sar,
        })
    if not out:
        raise RuntimeError('Purchase Order Details produced no Business with Partners rows. Check the workbook headers/source export.')
    return out


def load_strategic_partners():
    """Read the changing strategic-partner list from the dedicated workbook.

    The source is intentionally parsed defensively because the business-owned sheet may be a
    one-column list with a title/header or a small exported table.  We first try common partner
    headers and then fall back to non-empty text cells across the workbook.  Order is preserved
    and duplicates are removed case-insensitively.
    """
    path=FILES.get('strategic')
    if not path:
        return []
    aliases={'Strategic Partner','Strategic Partners','Partner','Partner Name','Vendor Name','Name'}
    values=[]
    try:
        for row in iter_records(path,1,aliases):
            for key in aliases:
                v=clean(row.get(key))
                if v:
    ...
    if not values:
        skip_norms={_header_norm(x) for x in aliases}|{'nofiltersapplied','strategicpartnerlist','strategicpartnerslist'}
        with zipfile.ZipFile(path) as z:
            shared=load_shared(z)
            for sheet in _worksheet_names(z):
                with z.open(sheet) as fh:
                    for ev,e in ET.iterparse(fh,events=('end',)):
                        ...
                        for v in raw.values():
                            txt=clean(v)
                            n=_header_norm(txt)
                            if not txt or n in skip_norms:
                                continue
                            # Ignore obvious numeric/date-only cells from decorative report areas.
    ...
    for v in values:
        v=clean(v)
        normalized=_header_norm(v)
        # Row labels such as "Strategic Partner 1" are not partner names.
        if normalized in {'strategic','strategicpartner','strategicpartners','strategicpartnerlist','strategicpartnerslist'}:
            continue
        if re.fullmatch(r'strategicpartner\d+', normalized):
            continue
        k=v.casefold()
...
def load_po_from_details(details):
    """Build the compact purchase-order relationship layer from Purchase Order Details.

    Purchase Order Details is now the authoritative PO source. This projection preserves
    the dimensions required by Order Book vendor/partner enrichment while de-duplicating
    PO-detail line items.
    """
    out=[]; matches=0; seen=set()
    for r in details or []:
        po=clean(r.get('purchaseOrder'))
        cid=clean(r.get('contractId'))
        supplier=clean(r.get('supplierName'))
        vendor_name=clean(r.get('vendorName'))
        vendor_type=clean(r.get('vendorType'))
        if not po and not cid:
            continue
        sig=(po.casefold(),cid.casefold(),supplier.casefold(),vendor_name.casefold(),clean(r.get('projectId')).casefold())
        if sig in seen:
            continue
        seen.add(sig)
        d=clean(r.get('accountingDate'))
        if d: matches+=1
        po_sar=num(r.get('poSAR'))
        out.append({
            'purchaseOrder':po or 'Unassigned',
            'accountingDate':d,
            'year':r.get('accountingYear'),
            'quarter':clean(r.get('accountingQuarter'),'Unassigned'),
            'month':clean(r.get('accountingMonth'),'Unassigned'),
            'vendorAccount':clean(r.get('vendorId'),'Unassigned'),
            # Current Purchase Order Details exposes Supplier Name separately from Vendor Name.
            # Keep Supplier Name as the legacy vendor, and Vendor Name as the partner relation.
            'vendor':supplier or vendor_name or 'Unassigned',
            'partner':vendor_name or ('Partner' if 'partner' in vendor_type.casefold() else 'Unassigned'),
            'partnerName':vendor_name or 'Unassigned',
            'market':clean(r.get('market'),'Unassigned'),
            'country':clean(r.get('country'),'Unassigned'),
            'vertical':clean(r.get('vertical'),'Unassigned'),
            'contractName':clean(r.get('contractName'),'Unassigned'),
            'contractId':cid or 'Unassigned',
            'projectId':clean(r.get('projectId'),'Unassigned'),
            'customer':clean(r.get('customerName'),'Unassigned'),
            'projectManager':'Unassigned',
            'contractCurrency':'SAR',
            'submittingUser':clean(r.get('submittingUser'),'Unassigned'),
            'currency':'SAR',
            'netAmount':po_sar,
            'netSAR':po_sar,
            'grossAmount':po_sar,
